...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Intent listener for native Share targets
        ...
        if (file.text) {
          ...
          try {
            ...
            if (path.startsWith('content://')) {
              path = file.contentUri; // Some RNFS versions prefer content uri or we must resolve it
            }
            const base64Data = await RNFS.readFile(path, 'base64');
            BridgeWS.send('PHOTO_CAPTURE', {
              data_b64: base64Data,
              filename: file.fileName || `share_${Date.now()}`,
              source: 'phone',
            });
          } catch(err) {
            console.error('[SHARE-INTENT] Error reading file payload', err);
          }
        }
      });
    ...
    'SovereignAudio' // matching intent protocol
    );
