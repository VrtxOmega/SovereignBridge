...
class BridgeWebSocketService {
  ...
  _setupNetInfoListener() {
    this._netInfoUnsubscribe = NetInfo.addEventListener(state => {
      if (state.isConnected && state.isInternetReachable !== false) {
        if (!this.connected || this.ws?.readyState !== WebSocket.OPEN) {
          console.log('[BRIDGE-WS] Network restored — forcing instant reconnect');
          this.forceReconnect();
        }
      }
    });
    ...
    try {
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === 'CLIPBOARD_UPDATE' && msg.source === 'pc') {
            try {
              Clipboard.setString(msg.content);
              console.log('[BRIDGE-WS] Auto-Synced PC Clipboard natively');
            } catch(clipboardErr) {
              console.error('[BRIDGE-WS] Failed setting android clipboard:', clipboardErr);
            }
          }
  ...
  _startHeartbeat() {
    ...
    this.pingInterval = setInterval(() => {
      if (this.ws?.readyState === WebSocket.OPEN) {
        this.ws.send(encryptPayload({ type: 'PING' }));
      }
    }, PING_INTERVAL);
    this._resetPongTimer();
  }
  ...
  // Public API
  send(type, payload = {}) {
    const msg = encryptPayload({ type, ...payload });
    if (this.connected && this.ws?.readyState === WebSocket.OPEN) {
      this.ws.send(msg);
    } else {
