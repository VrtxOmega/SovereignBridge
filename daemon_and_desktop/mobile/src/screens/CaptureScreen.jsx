...
const CaptureScreen = () => {
  ...
  const onCaptureLongPress = (capture) => {
    ...
    // If it has an image, add "Save Image" option
    if (capture.thumbnail_b64 || capture.format === 'image') {
      actions.unshift({
        text: 'Save Image',
        onPress: async () => {
          try {
            const b64 = capture.thumbnail_b64;
            if (b64) {
              const path = `${RNFS.PicturesDirectoryPath}/bridge_${Date.now()}.jpg`;
              await RNFS.writeFile(path, b64, 'base64');
              Alert.alert('Saved', `Image saved to Pictures`);
            }
          } catch (e) {
    ...
    Alert.alert(
      ...
      capture.content?.substring(0, 200),
      actions,
    );
  };
